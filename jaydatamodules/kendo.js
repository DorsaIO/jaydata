                ...
                init: function init(data) {
                    ...
                    if (options && options.owningContextType) {
                        contextSetTypes = options.owningContextType.memberDefinitions.getPublicMappedProperties().filter(function (pd) {
                            return pd.type ? ($data.Container.resolveType(pd.type) === $data.EntitySet) : false;
                        }).map(function (pd) {
                    ...
                    if (options.data.filter) {
                        ...
                        options.data.filter.filters.forEach(function (f, index) {
                            ...
                            if (thisfield.indexOf(".") > -1) {
                                thisfield = thisfield.substring(thisfield.lastIndexOf(".") + 1, thisfield.length);
                            }
                            switch (f.operator) {
                                case 'eq':
                                    filter += "it." + f.field;
                                    filter += " == this." + thisfield;
                                    break;
                                case 'neq':
                                    filter += "it." + f.field;
                                    filter += " != this." + thisfield;
                                ...
                                case 'startswith':
                                    filter += "it." + f.field;
                                    filter += ".startsWith(this." + thisfield + ")";
                                    break;
                                case 'contains':
                                    filter += "it." + f.field;
                                    filter += ".contains(this." + thisfield + ")";
                                    break;
                                case 'doesnotcontain':
                                    filter += "!";
                                    filter += "it." + f.field;
                                ...
                                case 'endswith':
                                    filter += "it." + f.field;
                                    filter += ".endsWith(this." + thisfield + ")";
                                    break;
                                case 'gte':
                                    filter += "it." + f.field;
                                    filter += " >= this." + thisfield;
